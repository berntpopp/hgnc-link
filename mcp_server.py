...
def main() -> None:
    """Run the hgnc-link MCP server on the stdio transport."""
    # Configure environment BEFORE importing anything that may print to stdout.
    os.environ.setdefault("HGNC_LINK_TRANSPORT", "stdio")
    ...
    try:
        ...
        from hgnc_link.server_manager import UnifiedServerManager
    except Exception as exc:
        print(f"ERROR: hgnc_link import failed: {exc}", file=sys.stderr)
        sys.exit(1)
