...
* Layer 1 -- ``on_call_tool`` registry preflight: ``get_tool(name)`` returns
  ...
  never echoes ``_meta.tool``. Runs OUTSIDE hgnc's ``ArgValidationMiddleware``.
* Layer 2 -- ``on_read_resource`` boundary: an unknown (URL-valid) resource makes
  ...
  URI-free ``ResourceError``. hgnc-link's resource handlers only return curated
  strings and raise no author-authored ``ResourceError``, so every exception here
  is a core not-found/read failure and is replaced unconditionally.
* Layer 3 -- protocol-handler backstop: wraps the raw ``CallTool`` / ``ReadResource``
  ...
  covers the unknown-PROMPT surface (hgnc-link registers no prompts).
...
(``middleware.py``), which reshapes pydantic call-binding errors for KNOWN tools.
Layer 6 (OTel span redaction) is a no-op here: hgnc-link declares no
``opentelemetry-sdk`` dependency, and the active tracer provider is the
non-recording ``ProxyTracerProvider`` (spans never record, so no span exception
attributes are ever captured) -- there is nothing to redact (fleet policy: do NOT
...
from hgnc_link.mcp._sanitize import sanitize_envelope, sanitize_message
...
# Fleet Response-Envelope Standard v1: stamp the clinical-safety disclaimer on
# EVERY tool response (success and error) -- mirror hgnc's envelope builder.
...
def unknown_tool_envelope() -> dict[str, Any]:
    ...
    Built from constants only; ``_meta`` carries a fresh ``request_id``, the
    fleet-standard ``unsafe_for_clinical_use`` disclaimer, and a discovery
    ``next_command`` but deliberately omits ``_meta.tool`` so the requested
    ...
    return sanitize_envelope(
        {
            ...
            "message": sanitize_message(_UNKNOWN_TOOL_MESSAGE),
            "retryable": False,
...
class NotFoundGuard(Middleware):
    ...
    async def on_call_tool(
        ...
        envelope. Only a fixed marker is logged -- the requested name is never
    ...
    async def on_read_resource(
        ...
        so the URI never reaches the caller/protocol. hgnc raises no
...
def _is_structured_envelope(call_result: mcp.types.CallToolResult) -> bool:
    ...
    Distinguishes a structured hgnc-link error (already input-free -- it carries
